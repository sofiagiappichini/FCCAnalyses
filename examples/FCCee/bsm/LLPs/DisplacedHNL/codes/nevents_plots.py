...
axs[0].legend(handles=legend_elements, loc='lower right', fontsize=12)

legend_elements = [Line2D([0], [0], color=color[i], lw=2, label=label[i]) for i in range(3,6)]
axs[1].legend(handles=legend_elements, loc='lower right', fontsize=12)
...
# Show all the plots
plt.tight_layout()
plt.savefig('/eos/user/s/sgiappic/www/plots/significance_final_points_events.png', format='png', dpi=330)