...
### code to validate samples produced with standalone madgraph+pythia+delphes ###
import numpy as np
import sys, os
import ROOT
...
inputfile = '/home/sofia/MG5_aMC_v3_5_0/delphes-3.5.1pre09/2HNL_1e-5_df_60gev.root'
chain.Add(inputfile)
          ...
          for i in range (daut1, daut2+1):
              if abs (branchGenPtcl.At(i).PID) == 11:       
                  ...
                  if branchGenPtcl.At(i).PT > 5:
                      branchjetEntries = 0 ### branchJet.GetEntries() ###
                      drmin = 10000
                      jetCounter = 0
