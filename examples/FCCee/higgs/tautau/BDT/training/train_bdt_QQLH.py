...
path = "/ceph/awiedl/FCCee/HiggsCP/stage2_100Coll150/QQ/LH/"

output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/BDT/output_QQLH.txt"
...
if train_tree == True:
    ...
    # Save the figure
    fig.savefig("/web/sgiappic/public_html/Higgs_xsec/BDT/ROC/QQLH_ROC_stage2_100Coll150.pdf")

    #Write model to joblib file
    joblib.dump(bdt, f"{out}/xgb_bdt_stage2_100Coll150_QQLH.joblib")
    ...
    ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_stage2_100Coll150_QQLH.root", num_inputs=len(vars_list))

    bdt.save_model(f"{out}/xgb_bdt_stage2_100Coll150_QQLH_model.json")
...
if full_test==True:
    ...
    bdt.load_model("xgb_bdt_stage2_QQLH_model.json")
    for q in bkgs:
        true_pred[q]=0
        false_pred[q]=0
        files = glob.glob(path+q+'/chunk_*.root')
