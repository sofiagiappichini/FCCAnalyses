...
if train_tree == True:
    ...
    # Save the figure
    fig.savefig("/web/sgiappic/public_html/BDT/NuNuLL_ROC_stage2_100Me.pdf")
    ...
    ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_stage2_100Me_NuNuLL.root", num_inputs=len(vars_list))

    bdt.save_model(f"{out}/xgb_bdt_stage2_100Me_NuNuLL_model.json")
...
if full_test==True:
    ...
    bdt.load_model("xgb_bdt_stage2_NuNuLL_model.json")
    for q in bkgs:
        true_pred[q]=0
        false_pred[q]=0
        files = glob.glob(path+q+'/chunk_*.root')
