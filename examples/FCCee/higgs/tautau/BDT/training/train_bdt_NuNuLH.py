...
path = "/ceph/awiedl/FCCee/HiggsCP/stage2_100Me/NuNu/LH/"

output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/BDT/output_NuNuLH.txt"
...
if train_tree == True:
    ...
    with open(output_file, "a") as file:
        ...
        file.write(f"Test: {x_test.shape}\n\n")
    ...
    # Save the figure
    fig.savefig("/web/sgiappic/public_html/BDT/NuNuLH_ROC_stage2_100Me.pdf")

    #Write model to joblib file
    joblib.dump(bdt, f"{out}/xgb_bdt_stage2_100Me_NuNuLH.joblib")
    ...
    ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_stage2_100Me_NuNuLH.root", num_inputs=len(vars_list))

    bdt.save_model(f"{out}/xgb_bdt_stage2_100Me_NuNuLH_model.json")
...
if full_test==True:
    ...
    bdt.load_model("xgb_bdt_stage2_NuNuLH_model.json")
    for q in bkgs:
        true_pred[q]=0
        false_pred[q]=0
        files = glob.glob(path+q+'/chunk_*.root')
