...
path = "/ceph/awiedl/FCCee/HiggsCP/stage2_100Me/NuNu/HH/"

output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/BDT/output_NuNuHH.txt"
...
if train_tree == True:
    ...
    # Save the figure
    fig.savefig("/web/sgiappic/public_html/BDT/NuNuHH_ROC_stage2_100Me.pdf")

    #Write model to joblib file
    joblib.dump(bdt, f"{out}/xgb_bdt_stage2_100Me_NuNuHH.joblib")
    ...
    ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_stage2_100Me_NuNuHH.root", num_inputs=len(vars_list))

    bdt.save_model(f"{out}/xgb_bdt_stage2_100Me_NuNuHH_model.json")
...
if full_test==True:
    ...
    bdt.load_model("xgb_bdt_stage2_NuNuHH_model.json")
    for q in bkgs:
        true_pred[q]=0
        false_pred[q]=0
        files = glob.glob(path+q+'/chunk_*.root')
