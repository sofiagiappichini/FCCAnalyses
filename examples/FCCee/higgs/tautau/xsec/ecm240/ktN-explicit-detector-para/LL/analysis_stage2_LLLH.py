...
outputDir = "/ceph/awiedl/FCCee/HiggsCP/detector_studies/ecm240/ktN-explicit/stage2_CMS2_para/LL/LH/"
    ...
    def analysers(df):
        ...
        for i in [0,1,2]:
            ...
            df=(df
                    ...
                    .Define(f"ThreeLeptons_{i}",    f"(((n_RecoElectrons_{i}==3 and n_RecoMuons_{i}==0) or (n_RecoElectrons_{i}==0 and n_RecoMuons_{i}==3)) and abs(RecoLepton_{i}_charge.at(0) + RecoLepton_{i}_charge.at(1) + RecoLepton_{i}_charge.at(2))==1)*1.0")
                    .Define(f"OnePair_{i}",     f"((n_RecoElectrons_{i}==2 and n_RecoMuons_{i}==1 and (RecoElectron_{i}_charge.at(0) + RecoElectron_{i}_charge.at(1))==0) or (n_RecoElectrons_{i}==1 and n_RecoMuons_{i}==2 and (RecoMuon_{i}_charge.at(0) + RecoMuon_{i}_charge.at(1))==0))*1.0")

                    .Filter(f"(ThreeLeptons_{i}==1 || OnePair_{i}==1)") 

                    .Define(f"ReconstructedParticlesJET_{i}",  f"FCCAnalyses::ReconstructedParticle::remove(ReconstructedParticles,RecoLeptons_{i})")
                
            )

            #EXCLUSIVE 2 JETS
            jetClusteringHelper_kt1  = ExclusiveJetClusteringHelper(f"ReconstructedParticlesJET_{i}", 1, f"kt1_{i}")

            df = jetClusteringHelper_kt1.define(df)

            df = (df
                    .Define(f"TagJet_kt1_{i}_px",           "JetClusteringUtils::get_px({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_py",           "JetClusteringUtils::get_py({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_pz",           "JetClusteringUtils::get_pz({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_p",           "JetClusteringUtils::get_p({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_pt",           "JetClusteringUtils::get_pt({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_phi",          "JetClusteringUtils::get_phi({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_eta",          "JetClusteringUtils::get_eta({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_theta",          "JetClusteringUtils::get_theta({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_e",       "JetClusteringUtils::get_e({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_mass",         "JetClusteringUtils::get_m({})".format(jetClusteringHelper_kt1.jets))
                    .Define(f"TagJet_kt1_{i}_charge",         "JetConstituentsUtils::get_charge_constituents({})".format(jetClusteringHelper_kt1.constituents))
                    .Define(f"TagJet_kt1_{i}_flavor",        "JetTaggingUtils::get_flavour({}, Particle)".format(jetClusteringHelper_kt1.jets))
                    .Define(f"n_TagJet_kt1_{i}_constituents",        "JetConstituentsUtils::get_n_constituents({})".format(jetClusteringHelper_kt1.constituents))
                    .Define(f"n_TagJet_kt1_{i}_charged_constituents",        "JetConstituentsUtils::get_ncharged_constituents({})".format(jetClusteringHelper_kt1.constituents))
                    .Define(f"n_TagJet_kt1_{i}_neutral_constituents",        "JetConstituentsUtils::get_nneutral_constituents({})".format(jetClusteringHelper_kt1.constituents))
                    .Define(f"n_TagJet_kt1_{i}",           f"return int(TagJet_kt1_{i}_flavor.size())")
                    .Define(f"TagJet_kt1_{i}_cleanup",       "JetConstituentsUtils::cleanup_taggedjet({})".format(jetClusteringHelper_kt1.constituents))

                    .Define(f"TauFromJet_kt1_{i}", "FCCAnalyses::ZHfunctions::findTauInJet({})".format(jetClusteringHelper_kt1.constituents))
                    .Define(f"TauFromJet_kt1_{i}_type_sel", f"ReconstructedParticle::get_type(TauFromJet_kt1_{i})")
                    .Define(f"TauFromJet_kt1_{i}_tau",  f"TauFromJet_kt1_{i}[TauFromJet_kt1_{i}_type_sel>=0]") 
                    .Define(f"TauFromJet_kt1_{i}_p",        f"ReconstructedParticle::get_p(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_pt",       f"ReconstructedParticle::get_pt(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_px",       f"ReconstructedParticle::get_px(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_py",       f"ReconstructedParticle::get_py(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_pz",       f"ReconstructedParticle::get_pz(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_theta",    f"ReconstructedParticle::get_theta(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_phi",      f"ReconstructedParticle::get_phi(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_eta",      f"ReconstructedParticle::get_eta(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_y",        f"ReconstructedParticle::get_y(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_e",        f"ReconstructedParticle::get_e(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_charge",   f"ReconstructedParticle::get_charge(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_type",     f"ReconstructedParticle::get_type(TauFromJet_kt1_{i}_tau)")
                    .Define(f"TauFromJet_kt1_{i}_mass",     f"ReconstructedParticle::get_mass(TauFromJet_kt1_{i}_tau)")
                    .Define(f"n_TauFromJet_kt1_{i}",        f"TauFromJet_kt1_{i}_pt.size()")

                    .Define(f"TagJet_kt1_{i}_sel_e",       f"TagJet_kt1_{i}_e[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_p",       f"TagJet_kt1_{i}_p[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_pt",      f"TagJet_kt1_{i}_pt[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_px",      f"TagJet_kt1_{i}_px[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_py",      f"TagJet_kt1_{i}_py[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_pz",      f"TagJet_kt1_{i}_pz[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_eta",     f"TagJet_kt1_{i}_eta[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_theta",   f"TagJet_kt1_{i}_theta[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_phi",     f"TagJet_kt1_{i}_phi[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"TagJet_kt1_{i}_sel_mass",    f"TagJet_kt1_{i}_mass[TauFromJet_kt1_{i}_type_sel<0]")
                    .Define(f"n_TagJet_kt1_{i}_sel",       f"TagJet_kt1_{i}_sel_e.size()")
            ...
            df = (df
                    ...
                    .Filter(f"n_TauFromJet_kt1_{i}==1 && n_TagJet_kt1_{i}_sel==0") 
                    ...
                    .Define(f"RecoZH_{i}_idx",        f"FCCAnalyses::ZHfunctions::FindBest_3(RecoLepton_{i}_p4, RecoLepton_{i}_charge, RecoLepton_{i}_mass, 91.188)")

                    ##################
                    #### FILTER 3 ####
                    ##################

                    .Filter(f"(TauFromJet_kt1_{i}_charge.at(0) + RecoLepton_{i}_charge.at(RecoZH_{i}_idx[2]))==0")
                    .Filter(f"RecoZH_{i}_idx[0]>=0 && RecoZH_{i}_idx[1]>=0 && RecoZH_{i}_idx[2]>=0")
                    ...
                    .Define(f"RecoTau2_{i}_p4",         f"TLorentzVector(TauFromJet_kt1_{i}_px.at(0), TauFromJet_kt1_{i}_py.at(0), TauFromJet_kt1_{i}_pz.at(0), TauFromJet_kt1_{i}_e.at(0))")
                    .Define(f"RecoTau1_{i}_type",       f"if (RecoLepton_{i}_mass.at(RecoZH_{i}_idx[2])<0.05) return float(-0.11); else return float(-0.13);")
                    .Define(f"RecoTau2_{i}_type",       f"float(TauFromJet_kt1_{i}_type.at(0))")

                    .Define(f"TauLepton_{i}_type",      f"if (RecoLepton_{i}_mass.at(0)<0.05) return float(-0.11); else return float(-0.13);")
                    .Define(f"TauHadron_{i}_type",      f"float(TauFromJet_kt1_{i}_type.at(0))")
    ...
    def output():
        ...
        for i in [0,1,2]:
            branchList += [
                ...
                f"TagJet_kt1_{i}_p",  
                f"TagJet_kt1_{i}_pt",    
                f"TagJet_kt1_{i}_phi", 
                f"TagJet_kt1_{i}_eta",     
                f"TagJet_kt1_{i}_theta",          
                f"TagJet_kt1_{i}_e",     
                f"TagJet_kt1_{i}_mass",        
                f"TagJet_kt1_{i}_charge",       
                f"TagJet_kt1_{i}_flavor", 
                f"n_TagJet_kt1_{i}_constituents",   
                f"n_TagJet_kt1_{i}_charged_constituents",   
                f"n_TagJet_kt1_{i}_neutral_constituents",   
                f"n_TagJet_kt1_{i}",          

                f"TauFromJet_kt1_{i}_p",
                f"TauFromJet_kt1_{i}_pt",
                f"TauFromJet_kt1_{i}_px",
                f"TauFromJet_kt1_{i}_py",
                f"TauFromJet_kt1_{i}_pz",
                f"TauFromJet_kt1_{i}_theta",
                f"TauFromJet_kt1_{i}_phi",
                f"TauFromJet_kt1_{i}_e",
                f"TauFromJet_kt1_{i}_eta",
                f"TauFromJet_kt1_{i}_y",
                f"TauFromJet_kt1_{i}_charge",
                f"TauFromJet_kt1_{i}_type",
                f"TauFromJet_kt1_{i}_mass",
                f"n_TauFromJet_kt1_{i}",

                f"TagJet_kt1_{i}_sel_e",     
                f"TagJet_kt1_{i}_sel_p",     
                f"TagJet_kt1_{i}_sel_pt",     
                f"TagJet_kt1_{i}_sel_px",   
                f"TagJet_kt1_{i}_sel_py",   
                f"TagJet_kt1_{i}_sel_pz",     
                f"TagJet_kt1_{i}_sel_eta",    
                f"TagJet_kt1_{i}_sel_theta",   
                f"TagJet_kt1_{i}_sel_phi",     
                f"TagJet_kt1_{i}_sel_mass",      
                f"n_TagJet_kt1_{i}_sel",
            ]
