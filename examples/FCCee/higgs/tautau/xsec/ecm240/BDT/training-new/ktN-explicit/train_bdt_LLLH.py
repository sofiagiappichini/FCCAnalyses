...
path = "/eos/experiment/fcc/ee/analyses_storage/Higgs_and_TOP/HiggsTauTau/ecm240/stage1_250502/ktN-explicit/LL/LH/"

out = "/eos/experiment/fcc/ee/analyses_storage/Higgs_and_TOP/HiggsTauTau/ecm240/BDT_250502/ktN-explicit/"
output_file = out + "/output_LLLH.txt"
...
joblib.dump(df_train, out + 'train_data_ktN-explicit_LLLH.pkl')
joblib.dump(df_test, out+ 'test_data_ktN-explicit_LLLH.pkl')
...
joblib.dump(bdt, f"{out}/xgb_bdt_ktN-explicit_LLLH.joblib")

#Also dump as json for ROOT interpretation
#booster = bdt.get_booster()
#booster.dump_model(f"{out}/xgb_bdt_stage2_cut_LLLH.json", dump_format='json')

# comment TMVA form output. TMVA Experimental only supports binary at the moment.
print("Writing xgboost model to ROOT file")
ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_ktN-explicit_LLLH.root", num_inputs=len(vars_list))

bdt.save_model(f"{out}/xgb_bdt_ktN-explicit_LLLH_model.json")
