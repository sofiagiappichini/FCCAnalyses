...
path = "/ceph/awiedl/FCCee/HiggsCP/ktN-tag/stage2_241202_cut/QQ/LH/"
...
if train_tree == True:
    ...
    # Save the figure
    fig.savefig("/web/awiedl/public_html/ML/BDT/QQLH_ROC_ktN-tag_stage2_241202_cut.pdf")

    #Write model to joblib file
    joblib.dump(bdt, f"{out}/xgb_bdt_ktN-tag_stage2_241202_cut_QQLH.joblib")
    ...
    ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_ktN-tag_stage2_241202_cut_QQLH.root", num_inputs=len(vars_list))

    bdt.save_model(f"{out}/xgb_bdt_ktN-tag_stage2_241202_cut_QQLH_model.json")
...
if full_test==True:
    ...
    bdt.load_model("xgb_bdt_ktN-tag_stage2_241202_cut_QQLH_model.json")
    for q in bkgs:
        true_pred[q]=0
        false_pred[q]=0
        files = glob.glob(path+q+'/chunk_*.root')
