...
sigs_VBF = ['wzp6_ee_VBF_nunuH_Htautau_ecm365']
...
path = "/eos/experiment/fcc/ee/analyses_storage/Higgs_and_TOP/HiggsTauTau/ecm365/stage1_250502/ktN-tag/NuNu/LH/"

out = "/eos/experiment/fcc/ee/analyses_storage/Higgs_and_TOP/HiggsTauTau/ecm365/BDT_250502/ktN-tag/"
output_file = out + "/output_NuNuLH.txt"
...
for q in sigs_ZH + sigs_VBF:
    ...
    for file in files:
        ...
        temp_df = temp_df[temp_df["RecoEmiss_e"] > 180]
    ...
    with open(output_file, "a") as file:
        file.write(f"Number of events in the dataframe of {q}: {len(df_sig_ZH + df_sig_VBF) - prev}\n\n")
        
with open(output_file, "a") as file:
    file.write(f"Total size of signal sample: {len(df_sig_ZH + df_sig_VBF)}\n")
print(f"Total size of signal sample: {len(df_sig_ZH + df_sig_VBF)}\n")

N_sig = len(df_sig_ZH + df_sig_VBF)
tot_weight_sig = df_sig_ZH["weight"].sum() + df_sig_VBF["weight"].sum()
...
joblib.dump(df_train, out + 'train_data_ktN-tag_NuNuLH.pkl')
joblib.dump(df_test, out+ 'test_data_ktN-tag_NuNuLH.pkl')
...
joblib.dump(bdt, f"{out}/xgb_bdt_ktN-tag_NuNuLH.joblib")
...
ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_ktN-tag_NuNuLH.root", num_inputs=len(vars_list))

bdt.save_model(f"{out}/xgb_bdt_ktN-tag_NuNuLH_model.json")
