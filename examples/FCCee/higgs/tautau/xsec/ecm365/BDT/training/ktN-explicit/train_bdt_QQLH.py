...
path = "/ceph/sgiappic/HiggsCP/ecm365/ktN-explicit/stage2_280125_cut/QQ/LH/"

output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/ecm365/BDT/outputs/ktN-explicit/output_QQLH.txt"

out = '/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/ecm365/BDT/models/ktN-explicit/' 

model = f"{out}/xgb_bdt_ktN-explicit_QQLH.joblib"
...
for i in ['p8_ee_WW_ecm365']:
    N_WW =  N_sig_new * weight[i] / tot_weight_bkg
    N_WW_min = min(N_WW, N[i])
    N_sig_new = min(N_sig_new, N_WW_min)
...
#booster.dump_model(f"{out}/xgb_bdt_stage2_cut_QQLH.json", dump_format='json')

# comment TMVA form output. TMVA Experimental only supports binary at the moment.
print("Writing xgboost model to ROOT file")
ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_ktN-explicit_QQLH.root", num_inputs=len(vars_list))

bdt.save_model(f"{out}/xgb_bdt_ktN-explicit_QQLH_model.json")
...
# Plot the ROC curve
plt.plot(fpr, tpr, lw=1.5, color="k", label=f'Htautau ROC (area = {roc_auc:.3f}) for QQLH')
...
fig.savefig("/web/sgiappic/public_html/Higgs_xsec/ecm365/ktN-explicit/BDT/QQLH_ROC_ktN-explicit.pdf")
