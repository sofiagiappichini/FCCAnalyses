...
path = "/ceph/sgiappic/HiggsCP/ecm365/ktN-explicit/stage2_280125_cut/QQ/LL/"

output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/ecm365/BDT/outputs/ktN-explicit/output_QQLL.txt"

out = '/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/ecm365/BDT/models/ktN-explicit/' 

model = f"{out}/xgb_bdt_ktN-explicit_QQLL.joblib"
...
#booster.dump_model(f"{out}/xgb_bdt_stage2_cut_QQLL.json", dump_format='json')

# comment TMVA form output. TMVA Experimental only supports binary at the moment.
print("Writing xgboost model to ROOT file")
ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_ktN-explicit_QQLL.root", num_inputs=len(vars_list))

bdt.save_model(f"{out}/xgb_bdt_ktN-explicit_QQLL_model.json")
...
# Plot the ROC curve
plt.plot(fpr, tpr, lw=1.5, color="k", label=f'Htautau ROC (area = {roc_auc:.3f}) for QQLL')
...
fig.savefig("/web/sgiappic/public_html/Higgs_xsec/ecm365/ktN-explicit/BDT/QQLL_ROC_ktN-explicit.pdf")
