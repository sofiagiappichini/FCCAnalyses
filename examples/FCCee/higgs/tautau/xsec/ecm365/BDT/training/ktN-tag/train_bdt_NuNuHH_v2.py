...
output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/ecm365/BDT/outputs/ktN-tag/output_NuNuHH_v2.txt"
...
for q in sigs_ZH:
    ...
    df = pd.DataFrame()
    N[q], N_gen[q], eff[q], weight[q] = 0, 0, 0, 0
    ...
    for file in files:
        ...
        if f.keys()==['eventsProcessed;1']:
            continue
        tree = f["events"]
        gentree = f["eventsProcessed"]

        #compute the weight
        N[q] +=  tree.num_entries
        N_gen[q] +=  gentree.value
    ...
    with open(output_file, "a") as file:
        file.write(f"Weight of {q}: {df['weight'].iloc[0]}\n")
        #file.write(f"Relative weight of {q}: {(weight[q] / tot_weight_sig_ZH) }\n")
        #file.write(f"Requested size of {q}: {int(N_sig_ZH_new * weight[q] / tot_weight_sig_ZH) }\n")
        file.write(f"Events after stage2 of {q}: {N[q]}\n")
        file.write(f"Number of events in the dataframe of {q}: {(len(df_sig_ZH) - prev)}\n\n")
...
for q in sigs_VBF:
    ...
    if q=='wzp6_ee_numunumuH_Htautau_ecm365':
        df['weight'] = 1e-20
    else:
        df['weight'] = weight[q]
    ...
    with open(output_file, "a") as file:
        file.write(f"Weight of {q}: {df['weight'].iloc[0]}\n")
        #file.write(f"Relative weight of {q}: {((weight[q] / tot_weight_sig_VBF))}\n")
        #file.write(f"Requested size of {q}: {(int(N_sig_VBF_new * weight[q] / tot_weight_sig_VBF))}\n")
        file.write(f"Events after stage2 of {q}: {N[q]}\n")
        file.write(f"Number of events in the dataframe of {q}: {((len(df_sig_VBF) - prev))}\n\n")
...
for q in bkgs:
    ...
    print(f"backgorund done: {q}")

threshold = 1e-4 * len(df_bkg)

# Identify processes with fewer events than the threshold
df_bkg = df_bkg.groupby('process').filter(lambda x: len(x) >= threshold)
...
#booster.dump_model(f"{out}/xgb_bdt_stage2_cut_NuNuHH_v2.json", dump_format='json')

# comment TMVA form output. TMVA Experimental only supports binary at the moment.
print("Writing model")

# Save to a .root file in the TMVA format
ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_ktN-tag_NuNuHH_v2.root", num_inputs=len(vars_list))

bdt.save_model(f"{out}/xgb_bdt_ktN-tag_NuNuHH_model_v2.json")'''

#save the dataframes once they're created
joblib.dump((x_train, x_test, y_train, y_test), f"{out}/training_data_ktN-tag_NuNuHH.pkl")

#if model and dataframes are already present
#x_train, x_test, y_train, y_test = joblib.load(f"{out}/training_data_ktN-tag_NuNuHH.pkl")
...
fig.savefig("/web/sgiappic/public_html/Higgs_xsec/ecm365/ktN-tag/BDT/NuNuHH_ROC_v2.pdf")
