...
colorDict = ['#8C0303', '#D04747', '#FFABAC', '#03028D', '#4E6BD3', '#9FB5D7', '#4C9B3B', '#A9D18E']
    ...
    for cat in CAT:
        for sub in SUBDIR:
            ...
            if "QQ" in cat:
                ...
                if not os.path.exists(f"{modelDir}/training_data_{tag}_{cat}{sub}.pkl"):
                    N, N_gen, eff, weight = {}, {}, {}, {}
                    ...
                    # Train/Test split for separate signal categories
                    train_sig_ZH, test_sig_ZH = train_test_split(df_sig_ZH, test_size=0.3)
                    train_sig_VBF, test_sig_VBF = train_test_split(df_sig_VBF, test_size=0.3)
                    train_bkg, test_bkg = train_test_split(df_bkg, test_size=0.3)

                    #save the dataframes once they're created
                    joblib.dump((train_sig_ZH, test_sig_ZH, train_sig_VBF, test_sig_VBF, train_bkg, test_bkg), f"{modelDir}/training_data_{tag}_{cat}{sub}.pkl")
                else:
                    train_sig_ZH, test_sig_ZH, train_sig_VBF, test_sig_VBF, train_bkg, test_bkg = joblib.load(f"{modelDir}/training_data_{tag}_{cat}{sub}.pkl")

                # Load model
                bdt = joblib.load(f"{modelDir}/{tag}/xgb_bdt_{tag}_{cat}{sub}.joblib")

                # Predict on both signal categories and background
                pred_test_sig_ZH = bdt.predict_proba(test_sig_ZH[vars_list])
                pred_train_sig_ZH = bdt.predict_proba(train_sig_ZH[vars_list])
                pred_test_sig_VBF = bdt.predict_proba(test_sig_VBF[vars_list])
                pred_train_sig_VBF = bdt.predict_proba(train_sig_VBF[vars_list])

                pred_test_bkg = bdt.predict_proba(test_bkg[vars_list])
                pred_train_bkg = bdt.predict_proba(train_bkg[vars_list])

                # Predictions for signal classification (Label 1 for signal)
                p_test_sig_ZH = bdt.predict(test_sig_ZH[vars_list])
                p_train_sig_ZH = bdt.predict(train_sig_ZH[vars_list])
                p_test_sig_VBF = bdt.predict(test_sig_VBF[vars_list])
                p_train_sig_VBF = bdt.predict(train_sig_VBF[vars_list])

                p_test_bkg = bdt.predict(test_bkg[vars_list])
                p_train_bkg = bdt.predict(train_bkg[vars_list])
                
                # Accuracy scores for signal and background
                score_train_sig_ZH = accuracy_score(train_sig_ZH["label"], p_train_sig_ZH, normalize=True)
                score_test_sig_ZH = accuracy_score(test_sig_ZH["label"], p_test_sig_ZH, normalize=True)
                score_train_sig_VBF = accuracy_score(train_sig_VBF["label"], p_train_sig_VBF, normalize=True)
                score_test_sig_VBF = accuracy_score(test_sig_VBF["label"], p_test_sig_VBF, normalize=True)

                score_train_bkg = accuracy_score(train_bkg["label"], p_train_bkg, normalize=True)
                score_test_bkg = accuracy_score(test_bkg["label"], p_test_bkg, normalize=True)

                # Efficiency calculations
                eff_train_ZH, eff_test_ZH = [], []
                eff_train_VBF, eff_test_VBF = [], []
                eff_train_bkg, eff_test_bkg = [], []
                ...
                for j in range(3):
                    ...
                    for i in BDT_cuts:
                        ...
                        #efficiency for signal events considered as signal with various cuts on the 1-score in log scale
                        eff_train_ZH.append(max(1e-3, len([x for x in pred_train_sig_ZH[:, j] if x > cut_val])) / len(train_sig_ZH))
                        eff_test_ZH.append(max(1e-3, len([x for x in pred_test_sig_ZH[:, j] if x > cut_val])) / len(test_sig_ZH))
                        eff_train_VBF.append(max(1e-3, len([x for x in pred_train_sig_VBF[:, j] if x > cut_val])) / len(train_sig_VBF))
                        eff_test_VBF.append(max(1e-3, len([x for x in pred_test_sig_VBF[:, j] if x > cut_val])) / len(test_sig_VBF))
                        #efficiency for background events considered as signal
                        eff_train_bkg.append(max(1e-3, len([x for x in pred_train_bkg[:, j] if x > cut_val])) / len(train_bkg))
                        eff_test_bkg.append(max(1e-3, len([x for x in pred_test_bkg[:, j] if x > cut_val])) / len(test_bkg))

                    # Plotting
                    ax[j].plot(cut_vals, eff_train_ZH, color=colorDict[0], label=f'Trained ZH signal, accuracy: {round(score_train_sig_ZH, 3)}')
                    ax[j].plot(cut_vals, eff_test_ZH, color=colorDict[1], label=f'Tested ZH signal, accuracy: {round(score_test_sig_ZH, 3)}', linestyle='dashed', linewidth=1.5)
                    ax[j].plot(cut_vals, eff_train_VBF, color=colorDict[6], label=f'Trained VBF signal, accuracy: {round(score_train_sig_VBF, 3)}')
                    ax[j].plot(cut_vals, eff_test_VBF, color=colorDict[7], label=f'Tested VBF signal, accuracy: {round(score_test_sig_VBF, 3)}', linestyle='dashed', linewidth=1.5)
                    ax[j].plot(cut_vals, eff_train_bkg, color=colorDict[3], label=f'Trained background, accuracy: {round(score_train_bkg, 3)}')
                    ax[j].plot(cut_vals, eff_test_bkg, color=colorDict[4], label=f'Tested background, accuracy: {round(score_test_bkg, 3)}', linestyle='dashed', linewidth=1.5)

                    # Vertical line for last bin in BDT score
                    ax[j].axvline(x=-math.log10(1./200), color='k', label='Last bin in BDT score', linewidth=1.5, linestyle='-.')

                    ax[j].set_xticks([0, 1, 2, 3, 3.5])
                    ax[j].set_xticklabels(["$10^0$", "$10^{-1}$", "$10^{-2}$", "$10^{-3}$", " "])
                    ax[j].set_xlim(0, 3.5)
                    ax[j].set_yscale('log')
                    ax[j].set_ylim(10e-6, 1)
                    ax[j].grid()
                    
                    # Set subplot-specific labels
                    if j == 0:
                        ax[j].set_xlabel("1 - BDT background score", fontsize=18)
                        ax[j].set_ylabel("Efficiency", fontsize=18)
                    elif j == 1:
                        ax[j].set_xlabel("1 - BDT ZH score", fontsize=18)
                        ax[j].set_ylabel("Efficiency", fontsize=18)
                    elif j == 2:
                        ax[j].set_xlabel("1 - BDT VBF score", fontsize=18)
                        ax[j].set_ylabel("Efficiency", fontsize=18)

                    ax[j].legend(loc="upper right", fontsize=10)

                # Set common labels
                fig.suptitle(f'{tag}, {leg_cat[cat]} {leg_sub[sub]}: FCC-ee Simulation IDEA Delphes',fontsize=18, x=0.99, ha='right')


                plt.tight_layout()

                # Save the figure
                plt.savefig(f"/ceph/sgiappic/HiggsCP/ecm365/{tag}/BDT/{cat}{sub}_overtrain_all.pdf")
                plt.savefig(f"/ceph/sgiappic/HiggsCP/ecm365/{tag}/BDT/{cat}{sub}_overtrain_all.png")
