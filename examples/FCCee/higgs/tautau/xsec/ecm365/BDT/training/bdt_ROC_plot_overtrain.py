...
DIRECTORY = {
    'NuNu':"/ceph/awiedl/FCCee/HiggsCP/ktN-tag/stage2_241202_cut/",
    'QQ':"/ceph/awiedl/FCCee/HiggsCP/ktN-tag/stage2_241202_cut/",
    }
...
output_file = "/work/awiedl/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/BDT/output_plot.txt"

modelDir = "/work/awiedl/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/BDT/models/"
...
for cat in CAT:
    for sub in SUBDIR:
        ...
        elif "NuNu" in cat and "LH" in sub:
            vars_list = vars_list_NuNuLH
        elif "NuNu" in cat and "HH" in sub:
            vars_list = vars_list_NuNuHH
        elif "QQ" in cat and "LL" in sub:
            vars_list = vars_list_QQLL
        elif "QQ" in cat and "LH" in sub:
            vars_list = vars_list_QQLH
        elif "QQ" in cat and "HH" in sub:
            vars_list = vars_list_QQHH
        ...
        if cat=="NuNu":
            bdt = joblib.load(f"{modelDir}/xgb_bdt_ktN-tag_stage2_241202_cut_{cat}{sub}.joblib")
        else:
            bdt = joblib.load(f"{modelDir}/xgb_bdt_ktN-tag_stage2_241202_cut_{cat}{sub}.joblib")
        ...
        plt.plot(cut_vals, eff_train, color='b', label=f'Train {cat+sub} SIG ACC_SCORE:'+str(round(score_train_sig,3)))
        plt.plot(cut_vals, eff_test, color='g', label=f'Test {cat+sub} SIG ACC_SCORE:'+str(round(score_test_sig,3)), linestyle='dashed', linewidth='3.')
        plt.plot(cut_vals, eff_train_bkg, color='r', label=f'Train {cat+sub} BKG ACC_SCORE:'+str(round(score_train_bkg,3)))
        plt.plot(cut_vals, eff_test_bkg, color='tab:orange', label=f'Test {cat+sub} BKG ACC_SCORE:'+str(round(score_test_bkg,3)), linestyle='dashed', linewidth='3.')
        plt.axvline(x =-math.log10(1./100), color = 'k', label = 'CUT for BDT FIT')
        ...
        plt.tight_layout()
        plt.savefig("/web/awiedl/public_html/ML/BDT/241202/BDT_ktN-tag_overtraining"+cat+sub+".pdf")
        plt.clf()
        plt.close()
