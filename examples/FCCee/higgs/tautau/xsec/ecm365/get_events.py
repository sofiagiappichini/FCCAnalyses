...
for tag in TAG:
    ...
    for cat in CAT:
        for sub in SUBDIR:
            ...
            for file in all:
                ...
                # Sum entries for files containing "H_H"
                if file in signal:
                    if file not in table_data:
                        table_data[file] = {}
                    table_data[file][col_name] = f"{total_entries:.2e}" if total_entries != 0 else "0"
                elif file in ZH:
    ...
    if hh_total_data:
        table_data['ZH'] = {col: f"{value:.2e}" for col, value in hh_total_data.items()}
    ...
    # Save LaTeX Table
    latex_output_file = os.path.join(DIRECTORY, tag, "nevents.txt")
