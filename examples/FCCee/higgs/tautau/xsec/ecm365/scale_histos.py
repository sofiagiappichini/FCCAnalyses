...
for tag in TAG:
    for cat in CAT:
        for sub in SUBDIR:
            ...
            if "QQ" in cat and "R5-explicit" in tag:
                continue
            ...
            for file in os.listdir(directory):
                if file.endswith(".txt") or file.startswith("temp"):
                    continue
                else:
                    file_path = os.path.join(directory, file)
                    new_file_path = os.path.join(directory, "temp.root") 

                    outFile = ROOT.TFile.Open(new_file_path, "RECREATE")
                    tf = ROOT.TFile.Open(file_path, "READ")
                    for var in variables:
                        h = tf.Get(var)
                        if h:
                            hh = copy.deepcopy(h)
                            hh.SetDirectory(0)
                            hh.Scale(1.13) #from lumi 2.65 to 3 ab-1
                            outFile.cd()
                            hh.Write()  
                            print(f"{var} {tag} {cat}{sub}")
                    tf.Close()
                    outFile.Close()
                    os.replace(new_file_path, file_path)
                    print(f"Done: {tag} {cat}{sub} {file_path}\n")
