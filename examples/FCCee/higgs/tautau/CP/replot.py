...
DIR_PLOTS = '/web/sgiappic/public_html/HiggsCP/EEHH_id2/' 
...
for cut in CUTS:
    ...
    VARIABLES = VARIABLES_RECO + VARIABLES_CPRECO + VARIABLES_LL
        ...
        if nbkg!=0:
            ...
            for i in range(nsig):
                ...
                if i == 0:
                    ...
                    if LOGY==True :
                        h.GetYaxis().SetRangeUser(1e-1,1e6) #range to set if only working with signals
                    else:
                        max_y = h.GetMaximum() 
